...
- `funding.mjs` — Shared funding flow: `fundSessionKey()` (WalletConnect USDT + BNB transfer), `approveFacilitator()` (session-key broadcast of `ERC20.approve(facilitator, MaxUint256)`), `promptTopupAmount(minTopup, { couponMode, couponAmount })` (TTY-only preset picker, 支持优惠模式展示「套餐 / 实付」双值). Constants: `LOW_BALANCE_THRESHOLD = 1`, `MIN_TOPUP_USDT = 6`, `TOPUP_PRESETS = [6, 10, 20, 50]`, `AUTO_GAS_BNB = "0.0003"`, `COUPON_AMOUNT_USDT = 5` (与服务端 campaign 配置同步).
